...
const Carousel = () => {
  ...
  return (
    <div className="h-screen w-full -mt-[88px] relative overflow-hidden">
      <Slider {...settings}>
          ...
          <div key={slide.id} className="relative h-screen">
            ...
            >
              <div className="max-w-4xl">
                ...
                {/* Main Title */}
                <Title
                  className={`text-4xl md:text-6xl lg:text-7xl font-dancing mb-6 ${slide.titleColor} drop-shadow-2xl`}
                >
                  {slide.title}
                </Title>

                {/* Description */}
                <p className="text-white/90 text-base md:text-lg leading-relaxed mb-8 max-w-2xl drop-shadow-lg">
                  {slide.description}
                </p>

                {/* CTA Button */}
                <button className="btn-primary transform hover:scale-105 transition-all duration-300 shadow-2xl">
                  {slide.buttonText}
                </button>
              </div>
            </div>
            ...
            <div className="absolute bottom-20 left-8 opacity-20">
              <div className="w-24 h-24 border-4 border-white rounded-full animate-pulse"></div>
            </div>
            <div className="absolute top-32 right-8 opacity-20">
              <div className="w-16 h-16 border-4 border-primary rounded-full animate-bounce"></div>
            </div>
          </div>
        ))}
      </Slider>
      ...
      {/* Slide Counter */}
      <div className="absolute top-8 right-8 z-20 bg-black/50 backdrop-blur-sm rounded-full px-4 py-2 text-white text-sm">
